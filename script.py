...
for query in search_queries:
    try:
        ...
        m.send_keys("site:linkedin.com" + " AND " + str(query[0]) + " AND " + str(query[1]) )
        time.sleep(2)

        m.send_keys(Keys.ENTER)
        time.sleep(3)
        linkedin_url = driver.find_element_by_css_selector("div.yuRUbf > a").get_attribute('href')
        linkedin_url = linkedin_url.strip()
        result = [str(query[0]),str(query[1]),str(linkedin_url)]
        time.sleep(3)
